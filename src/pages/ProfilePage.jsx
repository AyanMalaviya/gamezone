...
function ProfileSection({ user, profile, onRefresh }) {
  ...
  const handleSave = async (e) => {
    ...
    if (phone.trim()) {
      ...
      if (!PH_RE.test(n)) {
        setStatus({ type:'err', msg:'Enter a valid 10-digit Indian mobile number.' });
        return;
      }
    }
    setBusy(true);
    ...
    <SectionCard title="Personal Info" icon={User} id="personal-info">
      <form onSubmit={handleSave} noValidate>
        ...
        <Field label="Email" icon={Mail} hint="Email cannot be changed here.">
          <input
            type="email" value={profile?.email || user?.email || ''}
            disabled
        ...
        <button type="submit" disabled={busy} style={{
          marginTop:16, width:'100%', padding:'.72rem',
          borderRadius:10,
          background:'linear-gradient(135deg,#7c3aed,#a855f7)',
          border:'none', color:'#fff', fontSize:'.88rem',
          ...
          {busy ? <Loader2 size={16} style={{ animation:'prof-spin .7s linear infinite' }} />
    ...
    <SectionCard title="Change Password" icon={Lock}>
      <form onSubmit={handleChange} noValidate>
        <Field label="Current Password" icon={Lock}>
          <input
            type={showC ? 'text' : 'password'} placeholder="Enter current password"
        ...
        <Field label="New Password" icon={Lock}>
          <input
            type={showN ? 'text' : 'password'} placeholder="Min. 6 characters"
        ...
        <div style={{ maxWidth:680, margin:'0 auto', padding:'calc(72px + 16px) clamp(16px,3vw,24px) 0' }}>
          <div style={{
            ...
            <div style={{
              ...
              <Phone size={16} color="#fca5a5" />
            </div>
            <div style={{ flex:1 }}>
              <p style={{ margin:0, fontSize:'0.82rem', fontWeight:700, color:'#fca5a5' }}>
                Phone number missing
              </p>
              <p style={{ margin:0, fontSize:'0.75rem', color:'rgba(255,255,255,0.4)', marginTop:2 }}>
                Required to book a station. Add it below in Personal Info.
              </p>
            </div>
            <a
              href="#personal-info"
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              <span style={{
                ...
                {hasPhone ? `📱 ${profile.phone}` : '⚠️ No phone — required for booking'}
              </span>
              <span style={{
                fontSize:'0.65rem', fontWeight:700, letterSpacing:'0.06em',
