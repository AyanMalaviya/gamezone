...
export default function AuthModal({ open, onClose, defaultTab = 'login' }) {
  ...
  const reset = () => { setEmail(''); setPass(''); setNameVal(''); setError(''); setBusy(false); setCaptchaOk(false); };
  ...
  const handleEmail = async (e) => {
    ...
    try {
      ...
      else                 await registerWithEmail(email, pass, nameVal.trim());
      handleClose();
    } catch (err) {
        ...
        >
          ...
          <div style={{ padding:'28px 28px 32px' }}>
            ...
            <form onSubmit={handleEmail} style={{ display:'flex', flexDirection:'column', gap:12 }}>
              ...
              {tab === 'register' && (
                <div style={{ position:'relative' }}>
                  <User size={15} style={{ position:'absolute', left:12, top:'50%', transform:'translateY(-50%)', color:'rgba(255,255,255,0.3)', pointerEvents:'none' }} />
                  <input
              ...
              {/* Phone-not-required hint — register only */}
              {tab === 'register' && (
                <div style={{
                  ...
                  fontSize:'0.75rem', color:'rgba(168,85,247,0.85)',
                }}>
                  <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
                    <circle cx="8" cy="8" r="7" stroke="#a855f7" strokeWidth="1.5"/>
                    <path d="M8 7v4M8 5.5v.5" stroke="#a855f7" strokeWidth="1.5" strokeLinecap="round"/>
                  </svg>
                  Phone number is optional — you can add it later from your profile.
                </div>
              )}
