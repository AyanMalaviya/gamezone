...
import { gasAddBooking } from '../lib/gasClient';
...
const usePaymentStore = create((set, get) => ({
  ...
  pay: async ({ upiId }) => {
    ...
    try {
      ...
      // 1. Save to Firestore — this is the only write needed.
      //    useStationData’s onSnapshot listener picks it up instantly,
      //    re-derives bookedSlots + activeSlot + status, and updates the UI.
      await saveBookingToFirestore(uid, context, receipt);

      // 2. Append audit row to Bookings sheet (fire-and-forget)
      gasAddBooking({
        txnId:       receipt.txnId,
        uid:         uid ?? '',
